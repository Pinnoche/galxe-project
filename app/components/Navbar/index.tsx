...
export default function Navbar() {
  ...
  const [winscreen, setWinscreen] = useState(window.innerWidth);

  useEffect(() => {
    window.onresize = () => {
      setWinscreen(window.innerWidth);
    };
  }, [winscreen]);
  ...
  return (
    <div
      className={`fixed bg-black w-full h-20 z-[7] ${
          ...
          <div className="w-full h-full flex tablet:flex-col">
            ...
            >
              ...
              >
                ...
                >
                  <span className="flex items-center tablet:justify-between cursor-pointer tablet:hover:text-white">
                    Explore
                ...
                <div
                  ...
                  onClick={() => setGalxeList((prev) => !prev)}
                >
                  <span className="flex items-center tablet:justify-between cursor-pointer hover:text-blue-700 tablet:hover:text-white">
                ...
                <div
                  ...
                  onMouseEnter={() => {
                    winscreen <= 1024
                      ...
                      : setProtocolList(true);
                  }}
                  onMouseLeave={() => setProtocolList(false)}
                  onClick={() => setProtocolList((prev) => !prev)}
                >
                  <span className="flex items-center tablet:justify-between cursor-pointer hover:text-blue-700 tablet:hover:text-white">
                ...
                <div
                  ...
                  onMouseEnter={() => {
                    winscreen <= 1024 ? setMoreList(false) : setMoreList(true);
                  }}
                  // onMouseLeave={() => setMoreList(false)}
                  onClick={() => setMoreList((prev) => !prev)}
                >
                  <span className="flex items-center cursor-pointer hover:text-blue-700 tablet:hover:text-white tablet:flex tablet:justify-between">
                    More <AiFillCaretDown className="h-4 w-7" />
                  </span>
                  {moreList && (
                    <div className="absolute top-[100%] pt-[30%] w-[710px] left-[-180px] tablet:relative tablet:pt-0 tablet:top-0 tablet:ml-0">
                      <div className="flex flex-col gap-0 bg-black border-gray-900 border-2 rounded-md overflow-hidden tablet:border-0 tablet:grid tablet:grid-cols-1">
              ...
              <div className="flex mr-4 ml-28 tablet:ml-4">
                <button className="bg-gray-700 text-gray-400 rounded-full text-xs py-[7px] px-6 mr-4 hover:bg-gray-500 tablet:hidden">
                  Network
                </button>
                <button
                  className={`bg-blue-700 rounded-full tablet:rounded-lg tablet:w-[100%] tablet:h-12 tablet:text-lg tablet:mb-8 py-[7px] px-6 text-xs text-white font-bold hover:bg-blue-500 ${
