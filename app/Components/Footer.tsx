            ...
            <div className='py-12 flex flex-wrap justify-between md:py-16 xl:py-20'>
                ...
                <div className="flex flex-wrap w-full xl:w-6/12 xl:justify-between">
                    {data.link_head.map((head:String,index:number) => (
                        <div className="w-1/2 mb-4  md:w-1/4 lg:w-1/5 xl:w-fit">
                            <p className="text-sm leading-6 font-normal text-white mb-3 font-bold ">{head}</p>
                            {data.links[index].map((link: String)=>(
                                <a href="/" className="block text-sm leading-6 font-normal text-footer-color mb-3 hover:text-footer-button-color cursor-pointer">{link}</a>
                            ))}
                        </div>
                    )    
                    )}
            ...
            <div className="flex py-6 flex-wrap border-footer2-bgx border-t lg:justify-between lg:py-8">
                <div className="flex flex-wrap justify-center w-full lg:w-2/3 lg:justify-normal">
                    {data.policies.map((policy: String) => (
                        <a href="/" className="w-fit cursor-pointer text-xs block leading-5 text-footer-light mx-3 underline">{policy}</a>
                    ))}
                </div>
                <div className="w-full lg:w-fit">
                    <p className="w-full text-center text-xs leading-5 text-footer-light mt-8 lg:w-fit lg:mt-0">{data.copyright}</p>
                </div>
            </div>
        </footer>
    );
